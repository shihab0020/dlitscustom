...
@frappe.whitelist()
def create_dlits_workspace_final():
    ...
    try:
        ...
        # Insert shortcuts
        shortcuts = [
            ('Pricing Rule Dlits', 'Pricing Rule Dlits', 'DocType', 'fa fa-tags', '#3498db', 'Advanced pricing rules with multiple base price types'),
            ('Sales Partner', 'Sales Partner', 'DocType', 'fa fa-handshake-o', '#e74c3c', 'Manage sales partners with commission tracking'),
            ('Payment Entry', 'Payment Entry', 'DocType', 'fa fa-credit-card', '#34495e', 'Process commission payments to sales partners'),
            ('Sales Order', 'Sales Order', 'DocType', 'fa fa-file-text-o', '#f39c12', 'Sales orders with pricing rule integration'),
            ('Sales Invoice', 'Sales Invoice', 'DocType', 'fa fa-file-text', '#27ae60', 'Sales invoices with commission calculation'),
            ('Item Price', 'Item Price', 'DocType', 'fa fa-money', '#8e44ad', 'Standard ERPNext item pricing management')
        ]
        ...
        for i, (label, link_to, type_val, icon, color, description) in enumerate(shortcuts):
            ...
            """, (shortcut_name, label, link_to, type_val, icon, color, description, i+1))
        ...
        return {
            "success": True,
            "message": "DLITS Custom workspace created successfully! You should now see it in your ERPNext interface.",
...
def check_workspace_status():
    ...
    try:
        ...
        shortcuts = frappe.db.sql("""
            SELECT label, link_to, type, icon, color, description
        ...
        return {
            ...
            "workspace": workspace,
            "shortcuts": shortcuts,
            "message": f"Workspace exists with {len(shortcuts)} shortcuts"
        }
