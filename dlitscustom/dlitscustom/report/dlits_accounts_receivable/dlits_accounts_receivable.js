...
frappe.query_reports["DLITS Accounts Receivable"] = {
	filters: [
		{
		...
		},
		{
			...
			options: get_dlits_party_type_options(),
			on_change: function () {
	...
	formatter: function (value, row, column, data, default_formatter) {
		...
		if (data && data.bold) value = value.bold();
		return value;
	},

	onload: function (report) {
		report.page.add_inner_button(__("DLITS AR Summary"), function () {
			var filters = report.get_values();
			frappe.set_route("query-report", "DLITS Accounts Receivable Summary", {
				company: filters.company,
				dlits_sales_partner: filters.dlits_sales_partner,
			});
		});
...
function get_dlits_party_type_options() {
	let options = [];
	frappe.db
		.get_list("Party Type", { filters: { account_type: "Receivable" }, fields: ["name"] })
		.then((res) => {
			res.forEach((pt) => options.push(pt.name));
		});
	return options;
}
