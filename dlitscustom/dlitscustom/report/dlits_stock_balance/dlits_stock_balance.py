...
class DlitsStockBalanceFilter(TypedDict):
	company: str | None
	from_date: str
	to_date: str
	brand: str | None
...
RUIJIE_BRAND = "RuijieReyee"
RUIJIE_WAREHOUSES = ["JD-KBW3-AbdulBari - ATE", "JD01-KBW-WH - ATE", "JD02-KBW-WH - ATE"]


def execute(filters: DlitsStockBalanceFilter | None = None):
	filters = frappe._dict(filters or {})
	if "Ruijie Viewer" in frappe.get_roles(frappe.session.user):
		filters.brand = RUIJIE_BRAND
		filters.warehouse = RUIJIE_WAREHOUSES
	return DlitsStockBalanceReport(filters).run()


class DlitsStockBalanceReport:
	def __init__(self, filters: DlitsStockBalanceFilter | None) -> None:
