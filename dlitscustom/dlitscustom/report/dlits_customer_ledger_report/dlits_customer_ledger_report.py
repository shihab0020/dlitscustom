...
def get_data(filters):
    ...
    if filters.get("customer"):
        customers = [{"name": filters.get("customer"), "customer_name": frappe.db.get_value("Customer", filters.get("customer"), "customer_name")}]
    else:
...
def get_customer_ledger_data(customer, from_date, to_date, filters):
    """Get ledger data for a specific customer"""
    ...
    # Payment Entries (Receipts from Customer)
    payment_entries = frappe.db.sql("""
        SELECT 
            ...
            party as customer,
            (SELECT customer_name FROM `tabCustomer` WHERE name = pe.party) as customer_name,
            0 as debit,
    ...
    journal_entries = frappe.db.sql("""
        SELECT 
            ...
            (SELECT customer_name FROM `tabCustomer` WHERE name = jea.party) as customer_name,
    ...
    for transaction in transactions:
        ...
        days_outstanding = ""
        if transaction.document_type == "Sales Invoice" and transaction.outstanding_amount > 0:
...
def get_opening_balance(customer, from_date):
    ...
    journal_total = frappe.db.sql("""
        SELECT 
            COALESCE(SUM(jea.debit_in_account_currency), 0) - COALESCE(SUM(jea.credit_in_account_currency), 0) as total
        FROM `tabJournal Entry` je
        INNER JOIN `tabJournal Entry Account` jea ON je.name = jea.parent
        WHERE jea.party_type = 'Customer'
        AND jea.party = %s
...
def get_conditions(filters):
    ...
    if filters.get("customer_group"):
        conditions += f" AND customer_group = '{filters.get('customer_group')}'"
...
def get_chart_data(data, filters):
    ...
    for row in data:
        if row.get("document_type") == "Sales Invoice" and row.get("days_outstanding"):
            days = int(row.get("days_outstanding", 0))
            outstanding_amount = flt(row.get("debit", 0)) - flt(row.get("credit", 0))
    ...
    return {
        "data": {
            "labels": ["Total Sales", "Total Payments", "Outstanding 0-30", "Outstanding 31-60", "Outstanding 61-90", "Outstanding 90+"],
            "datasets": [
                {
                    "name": "Customer Ledger Analysis",
                    "values": [total_debit, total_credit, outstanding_data["0-30"], outstanding_data["31-60"], outstanding_data["61-90"], outstanding_data["90+"]]
                }
            ]
        },
        "type": "bar",
        "height": 300,
        "colors": ["#28a745", "#17a2b8", "#ffc107", "#fd7e14", "#dc3545", "#6f42c1"]
    }