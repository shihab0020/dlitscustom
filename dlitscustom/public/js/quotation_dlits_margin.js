...
frappe.ui.form.on('Quotation', {
    onload(frm)  { _q_on_refresh(frm); },
    refresh(frm) { _q_on_refresh(frm); },
    items_remove(frm) { _q_schedule_sync(frm); },
...
frappe.ui.form.on('Quotation Item', {
    item_code(frm) { _q_schedule_sync(frm); },
    qty(frm)       { _q_schedule_sync(frm); },
    rate(frm)      { _q_schedule_sync(frm); },
});

// Guard: only fire for Quotation parent
frappe.ui.form.on('Dlits Margin Table Item', {
    cost(frm, cdt, cdn) {
        if (frm.doctype === 'Quotation') _dlits_recalc_row(frm, cdt, cdn);
    },
...
let _q_margin_timer = null;

function _q_on_refresh(frm) {
    if (frm.doc.docstatus === 1) {
        ...
        _q_schedule_sync(frm);
    } else {
        // Saved draft: Python validate already wrote correct rows; just render
        // DO NOT call _q_schedule_sync here — clear_table + add_child would dirty the form
        _dlits_render_summary(frm);
    }
}

function _q_schedule_sync(frm) {
    if (_q_margin_timer) clearTimeout(_q_margin_timer);
    _q_margin_timer = setTimeout(() => { _q_margin_timer = null; _dlits_sync_margin(frm); }, 600);
}

// ══ Shared implementation (identical in SO and SI files) ══════════════════════
