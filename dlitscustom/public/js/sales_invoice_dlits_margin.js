...
frappe.ui.form.on('Sales Invoice', {
    onload(frm)  { _si_on_refresh(frm); },
    refresh(frm) { _si_on_refresh(frm); },
    items_remove(frm) { _si_schedule_sync(frm); },
...
frappe.ui.form.on('Sales Invoice Item', {
    item_code(frm) { _si_schedule_sync(frm); },
    qty(frm)       { _si_schedule_sync(frm); },
    rate(frm)      { _si_schedule_sync(frm); },
});

// Guard: only fire for Sales Invoice parent
frappe.ui.form.on('Dlits Margin Table Item', {
    cost(frm, cdt, cdn) {
        if (frm.doctype === 'Sales Invoice') _dlits_recalc_row(frm, cdt, cdn);
    },
...
let _si_margin_timer = null;

function _si_on_refresh(frm) {
    if (frm.doc.docstatus === 1) {
        ...
        _si_schedule_sync(frm);
    } else {
        // Saved draft: Python validate already wrote correct rows; just render
        // DO NOT call _si_schedule_sync here — clear_table + add_child would dirty the form
        _dlits_render_summary(frm);
    }
}

function _si_schedule_sync(frm) {
    if (_si_margin_timer) clearTimeout(_si_margin_timer);
    _si_margin_timer = setTimeout(() => { _si_margin_timer = null; _dlits_sync_margin(frm); }, 600);
}

// ══ Shared implementation (identical in Quotation and SO files) ═══════════════
